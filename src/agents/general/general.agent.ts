...
import { transferToProvisionAgent, transferToRecommendationsAgent, transferToTerraformGeneratorAgent, transferToFinopsAgent, CheckHandOffToolFromMessages } from 'src/utils/createHandoffTool';
...
export const GeneralAgent = async (
  ...
  try {
    ...
    const command = await CheckHandOffToolFromMessages(response.messages, "general_agent");
    if (command) {
      return command;
    }
    const aiMessage = response.messages[response.messages.length - 1] as AIMessage;
    let responseContent = ""
    try {
      const parsedResponse = JSON.parse(aiMessage?.content?.toString() ?? '{}');
      responseContent = parsedResponse?.response?.toString() ?? 'No response generated.';
    } catch (error) {
      responseContent = aiMessage?.content?.toString() ?? 'No response generated.';
    }
    return {
      ...state,
      messages: [
