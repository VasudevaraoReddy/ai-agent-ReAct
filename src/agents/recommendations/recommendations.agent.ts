      ...
      messages: [
        new AIMessage(responseMessage, {
          ...
          details: {
            ...
            uniqueImpactedFields: formattedToolMessage?.uniqueImpactedFields,
            found: formattedToolMessage?.found,
          }
        }),
      ],
      extra_info: {
        ...state.extra_info,
        active_agent: 'recommendations_agent',
      },
    };
  } catch (error) {
    return {
...
const formatToolMessage = async (messages: ToolMessage[]) => {
  ...
  for await (const message of messages) {
    const content = JSON.parse(message?.content as string || '{}') ?? message.content;
    formattedMessages.recommendations = content?.recommendations ?? [];
    formattedMessages.uniqueImpactedFields = content?.uniqueImpactedFields ?? [];
