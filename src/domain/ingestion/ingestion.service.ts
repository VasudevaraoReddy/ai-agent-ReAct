...
import {
  ...
  createQdrantTextIndices,
} from 'src/utils/vector-store';
...
export class IngestionService implements OnModuleInit {
  ...
  constructor() {
    this.RECOMMENDATIONS_COLLECTION_NAME =
  ...
  async loadRecommendations() {
    try {
      const existingCollection = await this.checkCollectionExists(
        this.RECOMMENDATIONS_COLLECTION_NAME,
      );
      if (existingCollection) {
        console.log(
          `Collection ${this.RECOMMENDATIONS_COLLECTION_NAME} already exists`,
        );
      } else {
        console.log(
          `Creating collection ${this.RECOMMENDATIONS_COLLECTION_NAME}`,
        );
      ...
      const processBatch = async (startIndex: number): Promise<void> => {
        ...
        const batch = recommendations.slice(
          startIndex,
        ...
        console.log(
          `Upserting ${points.length} points to collection ${this.RECOMMENDATIONS_COLLECTION_NAME}`,
        );
      ...
      console.log(
        `Loaded recommendations into Qdrant collection ${this.RECOMMENDATIONS_COLLECTION_NAME}`,
      );
      return {
        success: true,
      ...
      console.error(
        `Error loading recommendations to ${this.RECOMMENDATIONS_COLLECTION_NAME}:`,
        error,
      );
      return false;
    }
