...
export const transferToRecommendationsAgent = createHandoffTool({
  ...
  description: 'Transfer user to recommendations agent when they need specific Azure service recommendations, best practices, or optimization suggestions for their Azure resources.',
});

export const transferToProvisionAgent = createHandoffTool({
  agentName: 'provision_agent',
  description: 'Transfer user to provision agent when they want to deploy, create, provision, or set up cloud resources or infrastructure services.',
});

export const transferToFinopsAgent = createHandoffTool({
  agentName: 'finops_agent',
  description: 'Transfer user to finops agent when they ask about cloud cost optimization, budgeting, cost analysis, resource right-sizing, or financial aspects of cloud management.',
});

export const transferToGeneralAgent = createHandoffTool({
  agentName: 'general_agent',
  description: 'Transfer user to general agent when they have general cloud computing questions about concepts, providers, technologies, or best practices.',
});

export const transferToTerraformGeneratorAgent = createHandoffTool({
  agentName: 'terraform_generator_agent',
  description: 'Transfer user to terraform generator agent when they want to generate Terraform code for infrastructure as code deployments or want to create IaC templates.',
});
...
export const CheckHandOffToolFromMessages = async (messages: BaseMessage[],from:string) : Promise<any> => {
  const toolMessage = messages.find((message) => message.getType() === 'tool' && message.name?.includes('transfer_to_') && !message.name?.includes(from as string));
