...
const recommendationsCollectionName =
  process.env.RECOMMENDATIONS_COLLECTION_NAME || '';

const recommendationsCollectionIndexKeys =
  process.env.RECOMMENDATIONS_COLLECTION_INDEX_KEYS || '';
...
export const createQdrantTextIndices = async (
  ...
  for (const collectionName of collectionNames) {
    ...
    try {
      ...
      const keysWithTokenizer = indexKeysEnvVar
        .split(',')
        .map((key) => key.split(':'));
